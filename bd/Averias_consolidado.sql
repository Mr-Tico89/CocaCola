...
SELECT DISTINCT ID, MES, SEMANA, FECHA, A±O, TURNO, MAQUINA, SINTOMA, AREAS, MINUTOS, OBSERVACIONES FROM DB_AVERIAS_CONSOLIDADO;

, , , , , , , OBSERVACIONES

CREATE TABLE valores_unicos_averias AS
SELECT 
    ARRAY(SELECT DISTINCT ID  FROM DB_AVERIAS_CONSOLIDADO) AS ID,
    ARRAY(SELECT DISTINCT MES FROM DB_AVERIAS_CONSOLIDADO) AS MES,
    ARRAY(SELECT DISTINCT SEMANA FROM DB_AVERIAS_CONSOLIDADO) AS SEMANA,
    ARRAY(SELECT DISTINCT FECHA FROM DB_AVERIAS_CONSOLIDADO) AS FECHA,
    ARRAY(SELECT DISTINCT A±O FROM DB_AVERIAS_CONSOLIDADO) AS A±O,
    ARRAY(SELECT DISTINCT TURNO FROM DB_AVERIAS_CONSOLIDADO) AS TURNO,
    ARRAY(SELECT DISTINCT MAQUINA FROM DB_AVERIAS_CONSOLIDADO) AS MAQUINA,
    ARRAY(SELECT DISTINCT SINTOMA FROM DB_AVERIAS_CONSOLIDADO) AS SINTOMA,
    ARRAY(SELECT DISTINCT AREAS FROM DB_AVERIAS_CONSOLIDADO) AS AREAS,
    ARRAY(SELECT DISTINCT MINUTOS FROM DB_AVERIAS_CONSOLIDADO) AS MINUTOS,
    ARRAY(SELECT DISTINCT OBSERVACIONES FROM DB_AVERIAS_CONSOLIDADO) AS OBSERVACIONES;
...
CREATE OR REPLACE FUNCTION actualizar_valores_unicos()
RETURNS TRIGGER AS $$
BEGIN
    DELETE FROM valores_unicos;
    INSERT INTO valores_unicos
    SELECT DISTINCT columna1, columna2, columna3 FROM averias;
    RETURN NULL;
END;
$$ LANGUAGE plpgsql;
...
CREATE TRIGGER trigger_actualizar_valores
AFTER INSERT OR UPDATE OR DELETE ON averias
FOR EACH STATEMENT
EXECUTE FUNCTION actualizar_valores_unicos();
